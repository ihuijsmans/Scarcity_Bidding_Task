...
%WD for Practice
stimuli_dir_practice = [cd '\Practice\'];
...
practice_wintrial = {1,5,7,8,11,13, 14, 15 17,21,23,25,26};
...
%Return stagegame
a_return_stagegame = imread([stimuli_dir_general 'a_return_stagegame.JPG']);
a_return_stagegame = Screen('MakeTexture', window, a_return_stagegame);
s_return_stagegame = imread([stimuli_dir_general 's_return_stagegame.JPG']);
s_return_stagegame = Screen('MakeTexture', window, s_return_stagegame);

c.return_stagegame = {s_return_stagegame, a_return_stagegame};

%Intructions shoptask
a_inst_shoptask = imread([stimuli_dir_general 'a_inst_shoptask.JPG']);
a_inst_shoptask = Screen('MakeTexture', window, a_inst_shoptask);
s_inst_shoptask = imread([stimuli_dir_general 's_inst_shoptask.JPG']);
s_inst_shoptask = Screen('MakeTexture', window, s_inst_shoptask);

c.inst_shoptask = {s_inst_shoptask, a_inst_shoptask};
...
U_end = imread([stimuli_dir_practice 'U_end.JPG']);
U_end = Screen('MakeTexture', window, U_end);

U_gameover = imread([stimuli_dir_practice 'U_game over.JPG']);
...
fprintf(fid_slider, '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n', 'trial', 'flip_nr', 'handle_x','response', 'fliptimestamp', 'tmp_bid','RT', 'VBLTimestamp', 'BidFlipTimestamp', 'FlipTimestamp', 'MissedFlip', 'ppnr', 'condition_txt', 'total_trial');
        ...
        while run
            ...
            switch screenID
                ...
                case 6
                    ...
                    DrawFormattedText(window,txt_nrtokens, 0.686*wth, 0.546*hth, color);
...
BEH_ShopTask_Practice_12_10(window, vars, ppnr, mainstart, 1, fid_newtask, fid_slider)

function Main_Experiment_MRI_01_06(ppnr, block, stagegame, nrtrialcounter, shoptasktrial)
    % Scarcity Games
    ...
    try 
        ...
        if ~restart
            save([results_dir sprintf('wintrial_%i.mat', ppnr)], 'trialpicker');   
        end
    ...
    try 
        while block < (length(c.condition)+1)   
            ...
            while stagegame < (length(c.c_game)+1)    
                ...
                tokens = c.tokens(c.condition(block));

                %Reset count & trackers for each block
                run = 1;
                
                %Reset nrtrial counter unless we just restarted the experiment
                if restart == 0
                    nrtrialcounter = 1;
                end
                ...
                while run
                    ...
                    switch screenID
                        ...
                        case 5
                            ...
                            switch nrtrialcounter
                                case c.wintrials(c.c_game(stagegame),:)
                        ...
                        case 6
                            ...
                            if nrtrialcounter == c.stop(c.c_game(stagegame))
                                screenID = 1.2;
                            end
                    ...
                    if screenID == 1.4
                        ...
                        load([results_dir sprintf('wintrial_%i.mat', ppnr)]); 
                        
                        %Shoptask
                        [trialpicker_block, manualclose] = BEH_ShopTask_01_06(window, vars, stagegame+((block-1)*3), shoptasktrial, ppnr, mainstart, c.reminder{c.condition(block)}, c.condition(block), fid_newtask, fid_slider, ft_id, SCANNER);
